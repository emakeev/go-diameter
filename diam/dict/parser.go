...
type Parser struct {
	...
	appcodetype map[appCodeTypeIdx]*App // Application index by code & type
...
func (p *Parser) Load(r io.Reader) error {
	...
	p.once.Do(func() {
		...
		p.appcodetype = make(map[appCodeTypeIdx]*App)
	...
	for _, app := range f.App {
		...
		p.appcodetype[appCodeTypeIdx{app.ID, app.Type}] = app
		// Cache commands.
